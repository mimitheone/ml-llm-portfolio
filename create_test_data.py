...
def create_transaction_data(n_samples=2000):
    ...
    merchant_categories = np.random.choice(
        ...
        n_samples,
        p=[0.3, 0.1, 0.2, 0.1, 0.1, 0.05, 0.05, 0.05, 0.05],
    )

    # Location patterns
    locations = np.random.choice(
        ["local", "national", "international"], n_samples, p=[0.7, 0.2, 0.1]
    )
    ...
    # Fraud indicators (not used in current implementation)
...
def create_recommendation_data(n_samples=1000):
    ...
    for user_id in user_ids:
        ...
        selected_products = np.random.choice(
            product_ids, num_interactions, replace=False
        )
