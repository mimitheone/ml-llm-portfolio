...
def main():
    ...
    for feature in cfg["features"]:
        ...
        print(
            f"   - {feature}: True={true_coef:.1f}, Predicted={pred_coef:.4f}, Error={error:.4f}"
        )
    ...
    print(f"\n🔧 To run the full CLI:")
    print(
        f"   python -m src.cli train --model linear_regression --task regression --config configs/regression/linear_regression.yaml"
    )
    print(
        f"   python -m src.cli evaluate --model linear_regression --task regression --config configs/regression/linear_regression.yaml"
    )
    print(
        f"   python -m src.cli predict --model linear_regression --task regression --config configs/regression/linear_regression.yaml --input data/processed/regression_inference.csv --output reports/predictions.csv"
    )
