...
from src.pipelines.classification import train_random_forest, evaluate as evaluate_classification, predict as predict_classification
from src.pipelines.regression import train_linear_regression, evaluate as evaluate_regression, predict as predict_regression
...
def main():
    ...
    p_train.add_argument("--model", required=True, choices=["random_forest", "linear_regression"])
    p_train.add_argument("--task", required=True, choices=["classification", "regression"])
    p_train.add_argument("--config", required=True)

    p_eval = sub.add_parser("evaluate", help="Evaluate a model")
    p_eval.add_argument("--model", required=True, choices=["random_forest", "linear_regression"])
    p_eval.add_argument("--task", required=True, choices=["classification", "regression"])
    p_eval.add_argument("--config", required=True)

    p_pred = sub.add_parser("predict", help="Predict with a model")
    p_pred.add_argument("--model", required=True, choices=["random_forest", "linear_regression"])
    p_pred.add_argument("--task", required=True, choices=["classification", "regression"])
